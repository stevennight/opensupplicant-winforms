...
namespace OpenSupplicant
{
    class ProtocalUtil
    {
        ...
        public static Dictionary<byte, List<byte[]>> ParseResponseBody(byte[] buf)
        {
            ...
            //  返回消息长度过长超出byte范围(0-255)。 16.02.28
            //  表示不是很明白怎么看。不过调试看到offset好像会达到279。
            //  所以后面加上了判断，超出部分不再处理add到ret。
            ...
            while (offset < buf.Length)
            {
                ...
                //offset为byte类型，不能大于255 16.02.28
                //尝试发现temp比较过小不能用。过大就hehe。
                //估计只看这个offset看不懂其它代码的我是弄不懂所以然了PAP。
                int temp = offset + fieldLength + 2;
                if (temp < 255)
                {
